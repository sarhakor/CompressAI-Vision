...
def calc_complexity_nn_part1_plyr(vision_model, img):
    ...
    kmacs = measure_kmacs(vision_model.backbone, imgs.tensor)
...
def calc_complexity_nn_part2_plyr(vision_model, data, dec_features):
    """
    Objective:
    - Pass only Tensor inputs to fvcore (FlopCountAnalysis).
    - Avoid CUDA assertion and JIT type issues caused by Detectron2 post-processing
      (e.g., topk, sorting, NMS) in proposal and ROI stages.
    - Measure FLOPs only for neural network components:
      (top_block, RPN head, ROI box head + predictor, and optional mask head).

    Assumptions:
    - `data` is a dict containing p2-p5 features, where each value has shape (C, H, W).
    - `vision_model.top_block(p5)` generates p6 in the form of (p6, ...) or [p6].
    - `proposal_generator` and `roi_heads` are the original modules used in the pipeline.
    """
...
def _flops_to_kmacs(total_flops: float) -> float:
    ...
    """
    return float(total_flops) / 2.0 / 1e3
...
def measure_kmacs(module: nn.Module, inputs, tag: str = None) -> float:
    ...
    with torch.no_grad():
        ...
        # Note: A large number of "uncalled modules" is expected because this wrapper
        # traces only the active backbone path for the current NN-part setting.
        # Layers such as YOLO heads, loss branches, and skipped conditional paths
        # are intentionally not executed, but the main convolutional blocks are
        # correctly tracked, which is sufficient for reliable FLOPs/KMAC estimation.
